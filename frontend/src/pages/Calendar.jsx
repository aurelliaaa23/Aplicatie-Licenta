...
export default function Calendar() {
  ...
  const salveazaProgramare = async () => {
    ...
    try {
      ...
      toast.success('Programare salvată cu succes');
      setShowForm(false);
        ...
        {selected && (
          <div className="card" style={{ alignSelf: 'start' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              <div>
                <div style={{ display: 'inline-block', background: TIP_COMISIE_COLOR[selected.tip_comisie], color: 'white', padding: '3px 12px', borderRadius: 20, fontSize: 12, fontWeight: 600, marginBottom: 10 }}>
                  {TIP_COMISIE_LABEL[selected.tip_comisie]}
                </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div className="card" style={{ width: '100%', maxWidth: 480, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div className="form-group">
              <label>ID Dosar *</label>
              <input type="number" className="form-input" placeholder="ex: 42"
                value={form.dosar_id} onChange={set('dosar_id')} />
              <p className="form-hint">Introduceți ID-ul numeric al dosarului.</p>
            </div>

            <div className="form-group">
              <label>Tip comisie *</label>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 8 }}>
              ...
              <button className="btn btn-primary" onClick={salveazaProgramare} disabled={saving}>
                {saving ? <><div className="loading-spinner" /> Se salvează...</> : 'Salvează programarea'}
              </button>
            </div>
          </div>
        </div>
